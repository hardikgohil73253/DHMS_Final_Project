    ...
    private static Thread getHandleThread() {
        Runnable crash_task = () -> {
            try {
                ...
                System.out.println("RM2 is going to shutdown");
...
//                            messsageToFront(bug_message.toString(), data.FrontIpAddress);
                            ...
                            Message message = new Message(data.sequenceId, response, "RM2", data.Function, data.userID, data.newAppointmentID, data.newAppointmentType, data.oldAppointmentID, data.oldAppointmentType, data.bookingCapacity);
                            lastSequenceID += 1;
    ...
    private static String requestToServers(Message input) throws Exception {
        ...
        try (DatagramSocket aSocket = new DatagramSocket()) {
            ...
            System.out.println("RM2 sending request to server:" + ' ' + port);
            ...
            String response = new String(reply.getData(), 0,
                    reply.getLength());
            System.out.println("RM2 received response from server: " + response);
            return response;
        } catch (SocketException e) {
            System.out.println("Socket: " + e.getMessage());
        } catch (IOException e) {
    ...
    public static void messsageToFront(String message, String FrontIpAddress) {
        ...
        try (DatagramSocket socket = new DatagramSocket(4322)) {
