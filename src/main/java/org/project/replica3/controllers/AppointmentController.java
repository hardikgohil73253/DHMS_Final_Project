package org.project.replica3.controllers;

import org.project.replica3.interfaces.WebServiceInterface;
import org.project.replica3.models.AppointmentModel;
import org.project.replica3.models.ClientModel;
import org.project.replica3.Logger;
...
@WebService(endpointInterface = "org.project.replica3.interfaces.WebServiceInterface")
...
public class AppointmentController implements WebServiceInterface {
    ...
    public  String listAppointmentAvailability(String appointmentType) {
        ...
        if (currentServerID.equals(VariableStore.SERVERS.MTL.toString())) {
            firstServer = broadcastMessageToOtherServers(VariableStore.QUEBEC_SOCKET_PORT3, "listAppointmentAvailability",
                    "null", appointmentType, "null");
            secondServer = broadcastMessageToOtherServers(VariableStore.SHERBROOKE_SOCKET_PORT3, "listAppointmentAvailability",
                    "null", appointmentType, "null");
        } else if (currentServerID.equals(VariableStore.SERVERS.SHE.toString())) {
            firstServer = broadcastMessageToOtherServers(VariableStore.QUEBEC_SOCKET_PORT3, "listAppointmentAvailability",
                    "null", appointmentType, "null");
            secondServer = broadcastMessageToOtherServers(VariableStore.MONTREAL_SOCKET_PORT3, "listAppointmentAvailability",
                    "null", appointmentType, "null");
        } else {
            firstServer = broadcastMessageToOtherServers(VariableStore.MONTREAL_SOCKET_PORT3, "listAppointmentAvailability",
                    "null", appointmentType, "null");
            secondServer = broadcastMessageToOtherServers(VariableStore.SHERBROOKE_SOCKET_PORT3, "listAppointmentAvailability",
                    "null", appointmentType, "null");
        }
    ...
    private String bookAppointmentForOtherCities(String patientID, String appointmentID,
        ...
        if (!checkWeeklySchedule(patientID, appointmentID.substring(4))) {
            String broadcasterResponse = broadcastMessageToOtherServers(
                    VariableStore.getSocketPortsForReplica3(AppointmentModel.detectAppointmentServer(appointmentID)), "bookAppointment",
                            ...
                            if (hashUserAppointment.containsKey(patientID)) {
                                if (hashUserAppointment.get(patientID).containsKey(appointmentType)) {
                                    ...
                                    String list = broadcastMessageToOtherServers(
                                            VariableStore.getSocketPortsForReplica3(
    ...
    private List<String> getAppointmentTypesForPatient(String appointmentID, String patientID, String appointmentType) {
        ...
        // Iterate through each primary key {PHYSICIAN, SURGEON, DENTAL}
        for (String primaryKey : hashAllAppointment.keySet()) {
            // Check if the appointment ID/ appointment on same day with different type
            if (hashUserAppointment.containsKey(patientID)) {
                if (hashUserAppointment.get(patientID).containsKey(primaryKey)) {
                    for (String appointment : hashUserAppointment.get(patientID).get(primaryKey)) {
                        ...
                        /*
                         * Primary used by if same server want to get if same appointment id is there
                         * with a user already.
                         */
                        ...
                        /*
                         * Primary used by if same server want to get if on same day appointment is
                         * there with a user already on different server.
                         */
                        if (!Objects.equals(appointmentType, primaryKey)) {

                            /*
                             * If we want to change that user can book appointment on same day but on
                             * different slot then we can change begin index
                             */
        ...
        if (!checkClientExists(patientID)) {
            ...
            if (ifAppointExistInUser(patientID, oldAppointmentType, oldAppointmentID)) {
                ...
                if (bookResp.startsWith("Success")
                    ...
                    response = "Failed: Your oldAppointment " + oldAppointmentID + " Could not be Canceled reason: "
                            + cancelResp;
                } else if (bookResp.startsWith("Fail")
                        && (cancelResp.startsWith("[Success") || cancelResp.startsWith("Success"))) {
                    String resp1 = bookAppointment(patientID, oldAppointmentID, oldAppointmentType);
                    response = VariableStore.APPOINTMENT_RESULT.Fail + ": Your newAppointment " + newAppointmentID
                            + " Could not be Booked reason: " + bookResp + " And your old Appointment Rolling back: " + resp1;
                } else {
                    response = VariableStore.APPOINTMENT_RESULT.Fail + ": on Both newAppointment " + newAppointmentID
                            + " Booking reason: " + bookResp + " and oldAppointment " + oldAppointmentID
                            + " Canceling reason: " + cancelResp;
                }
