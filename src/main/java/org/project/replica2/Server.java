...
import org.project.replica2.controllers.AppointmentController;
...
public class Server {
    ...
    private static void listenForClientRequest(AppointmentController appointmentController, int serverUdpPort, String serverID) {
        ...
        try {
            ...
            while (true) {
                 ...
                 * When server from one city communicates with other city's server
                 * */
    ...
    private void configServer(VariableStore.SERVERS serverLocation) {
        ...
        if (serverLocation.equals(VariableStore.SERVERS.MTL)) {
            this.serverPort = VariableStore.MONTREAL_SERVER_PORT2;
            this.serverBroadCastPort = VariableStore.MONTREAL_SOCKET_PORT2;
            serverEndpoint = url+this.serverPort+"/"+"montreal";
        } else if (serverLocation.equals(VariableStore.SERVERS.QUE)) {
            this.serverPort = VariableStore.QUEBEC_SERVER_PORT2;
            this.serverBroadCastPort = 7035;
            serverEndpoint = url+this.serverPort+"/"+"quebec";
        } else {
            this.serverPort = VariableStore.SHERBROOKE_SERVER_PORT2;
            this.serverBroadCastPort = VariableStore.SHERBROOKE_SOCKET_PORT2;
