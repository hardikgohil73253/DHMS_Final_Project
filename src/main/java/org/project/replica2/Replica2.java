...
public class Replica2 {
    ...
    private static void receive()  {
        ...
        try {
            ...
            System.out.println("RM2 UDP Server Started.");
...
//                message_q.clear();
    ...
    private static String requestToServers(Message input) throws Exception {
        ...
        try {
            ...
            URL montrealURL = new URL(url + VariableStore.MONTREAL_SERVER_PORT2 + "/"+ "montreal?wsdl");
            QName montrealQName = new QName("http://controllers.replica2.project.org/", "AppointmentControllerService");
            montrealWebService = Service.create(montrealURL, montrealQName);

            URL quebecURL = new URL(url + VariableStore.QUEBEC_SERVER_PORT2 + "/"+ "quebec?wsdl");
            QName quebecQName = new QName("http://controllers.replica2.project.org/", "AppointmentControllerService");
            quebecWebService = Service.create(quebecURL, quebecQName);

            URL sherbrookURL = new URL(url + VariableStore.SHERBROOKE_SERVER_PORT2 + "/"+ "sherbrook?wsdl");
            QName sherbrookQName = new QName("http://controllers.replica2.project.org/", "AppointmentControllerService");
    ...
    private static int serverPort(String input) {
        ...
        if (branch.equalsIgnoreCase("que"))
            portNumber = VariableStore.QUEBEC_SERVER_PORT2;
        else if (branch.equalsIgnoreCase("mtl"))
            portNumber = VariableStore.MONTREAL_SERVER_PORT2;
        else if (branch.equalsIgnoreCase("she"))
            portNumber = VariableStore.SHERBROOKE_SERVER_PORT2;
